...
from tschool import appmsg_sets, appmsg_sets2, capp_smsg, extserver
...
class myschool(extserver):
    SCHOOLS = {}
    #token = thirdsvr2.gen_token(cur_config.token())
    ...
    def settoken(cls, token):
        cls.token = extserver.token.copy()
        cls.token.update(token)
        if 'SecretKey' in cls.token:
            cls.SecretKey = cls.token.pop('SecretKey')
        elif SecretKey is None:
            raise ValueError("no SecretKey!")
        print(cls.token)

    @classmethod
    def myschool(cls, OrgId, chkable=False):
        if OrgId in cls.SCHOOLS:
            return cls.SCHOOLS[OrgId]
        else:
            _school = cls(OrgId, chkable=chkable)
            cls.SCHOOLS[OrgId] = _school
            return _school

    def __init__(self, school_OrgId, chkable=False):
        # student_department;
        # teacher_department;
        # objid <> OrgId
        self.token = self.__class__.token.copy()
        self.token['OrgId'] = school_OrgId
        self.objid = school_OrgId
        if chkable:
