...
function page_initor(mainpg, inifunc){
  ...
  page_collector = {
	...
	//on_clicks: [],
    inner_funcs: null,
	_dlg_onchange: function(evt){
		let _pger = this,
			_triggers = page_collector.triggers[this.id],
			$target = evt.target;
		let pckey=evt.target.dataset.pckey;
		if (!pckey){
			return;
		}
		let trigger = _triggers[pckey];
		if (!trigger){
			return;
		}
		if(trigger.mode === 'value' && ($target.nodeName==='INPUT' || $target.nodeName === 'SELECT')){
			//trigger.preval = trigger.value;
	...
	_dlg_onclick: function(evt){
		...
		if(trigger.mode === 'check' && ($target.type==='radio' || $target.type === 'checkbox')){
			trigger.value = $target.value;
			return true;
		}
	},
	
	setdatas: function(set, pageid){
		// set: object
		for(let k in set){
			this.$$(k, set[k]);
		}
	...
	settriggers: function(set, pageid){
		// set: {key: target(mode: value as default, if not input/select->text), key: {target: mode:...}}
		...
		for(let k in set){
			...
			if (typeof(d) === 'string'){
				...
				target = d['target'];
				value = d['value'];
				this.$$(k, target, mode, value);
			}
		}
	},
	
	collect_datas: function(keys, onlyexists){
		// 将当前页面指定数据按照{key: value, ...}收集
		if(typeof(keys) === 'string'){
			keys = keys.split(',');
		}
		let trigger = this.triggers[this.cur_page];
		let exportd = {};
		let _v;
		keys.forEach((v,i)=>{
			_d = trigger[v];
    ...
    bind_ini: function(pagename, func, at_once){
	  ...
	  // NEW+: change/click actions
	  // go for triggers
	  tpg.addEventListener("change", this._dlg_onchange);
    ...
    $: function(n, v, pg){
		// handle for batch set values:
		if (_exec.is_plainobj(n)){
			
			let pagename = v || this.cur_page;
			console.log("bath v for " + pagename);
			if(this.pgstats[pagename] === undefined){
				console.warn("page not found");
				return;
			}
			pagename = pagename + "_";
			for(_k in n){
				this.pagedatas[pagename + _k] = n[_k];
			}
			console.log(n);
			return;
		}
		...
		// get all values by page vname + pagename[arg2]
		if(n === null || n === undefined){
			// v as arg2 as pagename
			if(v === '_all'){
				return this.pagedatas;
			}
			pagename = (v || this.cur_page) + "_";
			let vs = {};
			for(k in this.pagedatas){
				if (k.startsWith(pagename)){
		...
		// get value from page
		inner_vname = pgname + n;
		// first deal with short way of readding:
		if (v === '_$del'){
			delete this.pagedatas[inner_vname];
			return true;
		} else if (v === undefined){
			return this.pagedatas[inner_vname];
		} else{
			this.pagedatas[inner_vname] = v;
			return v;
		}
	},
	$eq: function(key, value, pg){
		pg = (pg || this.cur_page) + "_";
		return this.pagedatas[pg+key] === value;
	},
			...
			_target.forEach((d,i)=>{
				...
				if(dom){
					switch($_.mode){
						...
						case 'stpl': {dom.innerHTML=enver($_.value, data)};break;
			...
			if(typeof(target)==='string'){
				...
				doms.forEach((d,i)=>{
					...
					if(dom){
						...
						}else if(mode === 'txtpl' || mode === 'stpl' || mode === 'innertpl'){
							//pass
						}else{
...
dlg_mgm = (function($){
	return{
		...
		*/
		...
		closedlg: function(dlgname, callback){
			let $dlg = this._$dlg(dlgname);
			$dlg.modal('hide');
			if (typeof(callback) === 'function'){
				callback();
		...
		set_values: function($dlg, vars_data, clear){
			...
			$tform.find('[name]').each(function(){
				...
				if(['text', 'number', 'tel', 'range', 'date', 'email', 'textarea', 'time'].indexOf(ttype) >= 0){
					this.value = (vval === 'None'?'':vval);
					console.log(this.name + "->" + vval);
		...
		get_json: function($dlg, item_names, item_mode, sp_ids, fillvals, skip_space){
			...
			$dlg = $dlg instanceof jQuery?$dlg:$($dlg);
			var $tform = $dlg.find("form");
			if($tform.length === 0){
			  $tform = $dlg;
			}
...
(function ($) {
  ...
  var tbl_funcs = {
	...
	ui_arange: function($tbl_obj, col_idx, isdesc, converter, row_from, row_end){
		...
		const def_charconv = (v)=>{
			let o=0;
			for(let i=0,len=v.length;i<len;i++){
				o += v.charCodeAt(i)*((len-i)*256);
			}
			console.log(v + "::" + o);
			return o;
		};
		const def_numinstr = (v)=>{
			let m = v.match(/.*?(\d+).*/);
			if (m){
				return parseInt(m[1]);
			}
		...
		// auto convert;
		let tbd = $tbl_obj instanceof jQuery?$tbl_obj.find('tbody')[0]:$tbl_obj.tBodies[0];
		if (!tbd){
		...
		if (!converter){
			let testv = trows[0].cells[col_idx].textContent;
			if (isNaN(parseFloat(testv))){
				console.log("no num test");
				if (def_numinstr(testv) === null){
					console.log("no num in str");
					converter = def_charconv;
				}else{
					converter = def_numinstr;
				}
			} else {
				converter = def_numconv;
			}
		} else if (typeof(converter === 'string')){
			switch(converter){
				case 'number':converter=def_numconv;break;
				case 'numinstr':converter=def_numinstr;break;
				case 'str':case 'string':converter=def_charconv;break;
				default: converter = undefined;
			}
		}
		if (converter){
			for(let i=0,len=row_end-row_from;i<len;i++){
				source.push(converter(trows[i].cells[col_idx].textContent));
			}
		} else{
			for(let i=0,len=row_end-row_from;i<len;i++){
				source.push(trows[i].cells[col_idx].textContent);
			}
		}
		//console.log(source);
		let mostsames = 0;
		if (isdesc){
			...
			for(let ise,x,v,i=0,len=source.length;i<len;i++){
				v = source[i];
				if(v === null){
					ranger.push(null);
					continue
		...
		for(let cc=0,i=0,len=ranger.length;i<len;i++){
			...
			for(let j=0;j<len;j++){
				if (ranger[j] === i){
					//console.log("working on: " + ranger[j]);
					target = _trows[j];
		...
		// final set
		let is_switch_happend;
		for(let x=0;x<10;x++){
			is_switch_happend = false;
			//console.log("final " + x + " times");
			for(let i=0,len=trows.length;i<len;i++){
				if (ranger[i] === null){continue}
				target = _trows[i];
				arch_node = trows[ranger[i]];
				if (target !== arch_node && target.cells[col_idx].textContent !== arch_node.cells[col_idx].textContent){
					//console.log("final switch on " + target.rowIndex + " before " + arch_node.rowIndex);
					tbd.insertBefore(target, arch_node);
					is_switch_happend = true;
					//break;
				}
			}
			if (is_switch_happend === false){
				console.log("no final switch, done");
				break;
			}
		}
	},
