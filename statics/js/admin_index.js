...
page_collector.bind_ini('attenders', function(){
	...
	this.querySelector("#btn_clear_all").onclick = function(){
		...
		$.post("/api/attenders?action=clear", {
			mid: _PGC.$("cur_meeting"),
			mtimes: -1, mcount: $("#attenders_tbl").data('stable').filter_params.mcount
		}, function(rsp){
			if(rsp && rsp.success === 'yes'){
				imsger.show("清除成功！");
...
page_collector.bind_ini("room_schedule", function(){
	...
	_PGC.settriggers({
		...
		cur_room_schedule: {target: 'schedule_listor', mode: 'innertpl', value: '<ul class="list-group">(for datas:<li class="list-group-item">会议名称：${name} | 召开时间：${nextdtime} | 时长：${mtime}分；</li>)</ul>'}
	});
	//按照单日查看
	this.querySelector("#btn_daybackw").onclick = function(){
		// move back data
		let fdate = new Date(_PGC.$$("on_date_s"));
		_PGC.$$("on_date_s", new Date(fdate.getTime() - 3600 * 24 * 1000).toISOString().substr(0,10));
		document.getElementById("schedule_single").click();
	}
	this.querySelector("#btn_dayforw").onclick = function(){
		// move back data
		let fdate = new Date(_PGC.$$("on_date_s"));
		_PGC.$$("on_date_s", new Date(fdate.getTime() + 3600 * 24 * 1000).toISOString().substr(0,10));
		document.getElementById("schedule_single").click();
	...
	document.getElementById("schedule_single").onclick = function(){
		...
		//console.log(ondate);
		$.getJSON("/resource/mroom?action=schedule&roomid=" + c_rid + '&ondate=' + ondate, function(rsp){
			if (rsp.success === 'yes'){
